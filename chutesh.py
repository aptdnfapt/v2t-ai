...
# --- Introduction ---
# This script implements an ultra-fast voice-to-text transcription service using optimized Chutes REST API calls.
#
# Speed Optimizations:
...
def transcribe_with_chutes(wav_data):
    """Transcribe WAV data using Chutes API."""
    log_message("Transcribing with Chutes API...")
    ...
    try:
        ...
        # Make the API call
        response = requests.post(CHUTES_API_URL, headers=headers, json=json_payload)

        api_time = time.time() - start_time
        log_message(f"API call to Chutes completed in {api_time:.2f}s with status code {response.status_code}")
        ...
        log_message(f"ERROR: HTTP error with Chutes API: {e}")
        ...
        log_message(f"ERROR: A request-related error occurred with Chutes API: {e}")
        log_message("This could be a network issue, DNS error, or timeout.")
        return None
    except Exception as e:
        log_message(f"ERROR: An unexpected error occurred while transcribing with Chutes API: {e}")
        return None
...
def process_audio_with_advanced_features(audio_data):
    ...
    try:
        ...
        transcribed_text = transcribe_with_chutes(audio_to_process)
        ...
        if transcribed_text:
            ...
            log_message("Transcription attempt failed.")
...
def main():
    ...
    # --- Pre-flight Checks ---
    if not CHUTES_API_TOKEN or CHUTES_API_TOKEN == "YOUR_API_TOKEN_HERE":
        log_message("ERROR: CHUTES_API_TOKEN environment variable not set.")
        log_message("Please create a .env file with CHUTES_API_TOKEN=\"YOUR_API_TOKEN\"")
        sys.exit(1)

    log_message("Chutes API token configured for ADVANCED FAST processing.")
    ...
    log_message(f"Features: Chutes API")
